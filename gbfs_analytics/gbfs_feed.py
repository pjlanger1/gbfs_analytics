...
METADATA_FIELDS = {'dc': {'station_status': [
    'is_installed',
    'station_id',
    'is_returning',
    'is_renting',
 ...
 'toronto': {'station_status': [
    'station_id',
    'is_charging_station',
    'status',
    'is_installed',
    'is_renting',
    'is_returning'],'free_bike_status':[None]
    }}
...
class gbfs_feed:
    ...
    def get_feed_info(self):
        ...
        feeds = {feed['name']: feed['url'] for feed in response.get('data', {}).get('en', {}).get('feeds', [])}
    ...
    def perform_snapshot(self, feed, interval, iterations, save_mode=False):
        ...
        timeseries = GBFSTimeSeries(feed, self.get_metadata_fields(feed))  # Create new timeseries instance
        self.timeseries_store[feed] = timeseries  # Store instance in dictionary
        task_counter = [0]  # Mutable counter to track iterations

        def task():
            print(f"Task started for {feed}: Iteration {task_counter[0] + 1}/{iterations}")  # Log the start of the task
            current_data = self.safe_request_handler(self.urls[feed])
            if current_data:
                timeseries.init_snapshot(current_data)  # Update timeseries with new data
                ...
                if save_mode:
                    ...
                    with open(f'{feed}_snapshot_{task_counter[0]}_{timestamp}.json', 'w') as f:
                        json.dump(current_data, f, indent=4)
                    print(f"Snapshot for {feed} saved at {time.ctime()}")  # Log snapshot timing

            task_counter[0] += 1  # Increment the counter after each iteration
            if task_counter[0] >= iterations:
                print(f"Maximum iterations reached for {feed}. Stopping scheduler.")  # Log when task is stopping
        ...
        print(f"Task for {feed} scheduled to run every {interval} seconds.")  # Log that scheduling was successful
    ...
    def perform_delta(self, feed, interval, iterations, save_mode=False):
        ...
        timeseries = GBFSTimeSeries(feed, self.get_metadata_fields(feed))  # Initialize with metadata fields
        self.timeseries_store[feed] = timeseries  # Store instance in dictionary
        task_counter = [0]  # Using a list to make it mutable inside the inner function
        old_data = [None]  # Use a list to hold old_data to avoid nonlocal declaration issues
        ...
        def task():
            ...
            if current_data:
                if old_data[0] is not None:
                    timeseries.init_delta(old_data[0], current_data)  #store delta in the timeseries object
                    delta = timeseries.calculate_delta(old_data[0], current_data)
                    #only save the delta if save_mode is True
                    if save_mode:
                        last_reported = datetime.utcfromtimestamp(
                            current_data['last_updated']).isoformat() + 'Z'
                        with open(f'{feed}_delta_{task_counter[0]}_{last_reported}.json', 'w') as f:
                            json.dump(delta, f, indent=4)
                        print(f"Delta for {feed} saved at {time.ctime()}")  #example logging
